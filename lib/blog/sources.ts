...
class Markdown {
  ...
  async *entries() {
    for await (const entry of walk(this.#folder, { exts: ["md"] })) {
      ...
      const properties = mapKeys(html.data.matter, (k) =>
        slugify(k, { lower: true })
      );
...
function selectTitle(pageBlock: { [key: string]: any }): string | undefined {
  const titleProperty = Object.values<any>(pageBlock.properties).find(
    (p) => p?.type === "title"
  );
...
class Drive {
  ...
  async *entries() {
    for await (const [meta, content] of this.#downloadAll(this.#fileId)) {
      ...
      const properties = mapKeys(file.data.matter, (k) =>
        slugify(k, { lower: true })
      );
